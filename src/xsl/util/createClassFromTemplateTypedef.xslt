		...
		<xsl:variable name="typeAndPassedBy">
			<xsl:choose>
				...
				<xsl:when
					...
					<!-- build list of type parameters -->
					<xsl:variable name="resolvedTypeParasInner">
						<xsl:call-template
					...
					<xsl:for-each select="$resolvedTypeParasInner/*">
						<xsl:message>      type para <xsl:value-of select="position()" />: <xsl:value-of select="." /></xsl:message>
					</xsl:for-each>
					 -->
					...
					<xsl:element name="type">
						<xsl:attribute name="const" select="if($infoNode/@addConst) then 'true'
								...
								$resolvedTypeParasInner/*[position() = $infoNode/@typeParaPos]/type/@const"/>
						<xsl:attribute name="constInTemplate" select="if ($type/@const = 'true')
																		then 'true' else 'false'"/>
						<xsl:value-of
							select="$resolvedTypeParasInner/*[position() = $infoNode/@typeParaPos]/type" />
					</xsl:element>
						...
						<xsl:choose>
							...
							<xsl:when
								test="($infoNode/@changePassedBy and $infoNode/@changePassedBy = 'true') and 
									($resolvedTypeParasInner/*[position() = $infoNode/@typeParaPos]/@passedBy = 'value') and
									($type/../@passedBy = 'value')">
								<xsl:value-of
							...
							<xsl:when
								test="(not($infoNode/@changePassedBy) or $infoNode/@changePassedBy = 'false') and 
									(not($resolvedTypeParasInner/*[position() = $infoNode/@typeParaPos]/@passedBy = 'value')) and
									($type/../@passedBy = 'value')">
								<xsl:value-of
									select="$resolvedTypeParasInner/*[position() = $infoNode/@typeParaPos]/@passedBy" />
							</xsl:when>
